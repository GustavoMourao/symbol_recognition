...
# For calculate euclidean distance
from math import sqrt
from math import fabs
# Visualize graph of features
import matplotlib.pyplot as plt
import matplotlib
# Sleep until storage values
import time
# Load model
...
class LeapMotionListener(Leap.Listener):
	...
	def on_frame(self, controller):
		...
		#plt.ion()
		i = 0
		### Create file, then write first line equals to zero
		#file = open("FeaturesTest.txt","w")
		#file.write("0\n")
		...
		# Counter variable: initial based on frame
		#frame.id
		#if FlagDifSimbol == False:
		#	Iinitial = frame.id
		...
		#for i in range(0,200):
			...
			arm = hand.arm
			#print ("Arm Direction: " + str(arm.direction) + " Wrist position(center of): " + str(arm.wrist_position) + " Elbow position: " + str(arm.elbow_position))
			
			
			#print ("PALM NORMAL::: " + str(normal[1])) # To initialize track: y>0.95!
			th = fabs(normal[1])
			# Track 500 points if NormYposition>.95
			if (th > 0.9) and (i < 500) and (FlagDifSimbol == False):
			
				#print ("Tracking activated...")
				
				# Get data for each individual finger and bone
				for finger in hand.fingers:
					# print ("Type: " + self.finger_names[finger.type] + " Length(mm): " + str(finger.length) + " Width(mm): " + str(finger.width) + " Direction: " + str(finger.direction))
					# print ("Type: " + str(finger.type) + " Length(mm): " + str(finger.length) + " Width(mm): " + str(finger.width))
					...
					DpalmtipTotal = DpalmtipThumb + DpalmtipIndex + DpalmtipMiddle + DpalmtipRing + DpalmtipPinky
					...
					#print (" RTP_0: " + str(RTP_0))
					# print ("Position Bone thumb: " + str(boneTIntermediate))
					...
					for b in range(0, 4):
						bone = finger.bone(b)
						if (bone.type == 0):   # Position 3: Metacarpo
						...
						if (bone.type == 1): # Position 2: Proximal
							if finger.type == 0:
						...
						if (bone.type == 2): # Position 1: Intermediate
							if finger.type == 0:
						...
						# print ("Bone: " + self.bone_names[bone.type] + " Direction: " + str(bone.direction))
						#print ("DTJ_ThumbJ3= " + str(DTJ_ThumbJ3))
						#print ("DTJ_IndexJ3= " + str(DTJ_IndexJ3))
						#print ("DTJ_RingJ1= "+ str(DTJ_RingJ1))
				...
				tic = time.clock()
				classifier = joblib.load('NearestCentroid.pkl')
				...
				if TotalLines < 500:
					...
					readfile = open("FeaturesTest.txt","r")
